...
class Inference(metadata_pb2_grpc.InferencerServicer):
    ...
    def Submit(self, request, context):
        ...
        print(" Process %s time taken: %s" % (process_id,end-start_s))
        #print("Process %s overhead %s" % (process_id,end-start_s - (end_i-start)))
        ov_head.append(end-start_s - (end_i-start))
        #sum += end-start_s
        #count += 1
        inf_dict[process_id].append(end_i-start)
        e2e_dict[process_id].append(end-start_s)
...
def start_server():
    ...
    try:
        ...
        print(statistics.mean(inf),statistics.mean(ov_head))
        with open("inf_result_tempfile_bodypose.json", "w") as json_file:
            json.dump(inf_dict, json_file)
        with open("e2e_result_tempfile_bodypose.json", "w") as json_file:
